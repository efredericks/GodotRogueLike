...
func _get_path(path_to_node: String) -> String:
	if "GUI" in path_to_node:

		#print("/root/PC/MainGUI")
		#return "/root/PC/MainGUI/{1}".format([_path_to_self, path_to_node])
		return "{0}/CanvasLayer/{1}".format([_path_to_self, path_to_node])
	else:
		return "{0}/{1}".format([_path_to_self, path_to_node])
	#return "{0}/Viewport/{1}".format([_path_to_self, path_to_node])
